...
const Form = ({ title, onError, onFinally, onSuccess }: Props) => {
    const { showPassword: showPass, setShowPassword } = useUtilsStore();
    const { form, onSubmit } = useLogin({ onError, onFinally, onSuccess });
    const { handleSubmit, register } = form;
    return <form className="max-w-lg w-full bg-white flex flex-col gap-5 rounded-lg shadow-sm p-5" onSubmit={handleSubmit(onSubmit)}>
